...
matrix
matrix<-rbind(mat1,mat2) #rbind : row를 기준으로 종으로 붙이기
matrix
#특정 위치의 요소 추출 및 치환
matrix[1,2]
matrix[1:2]
...
matrix[1,2]=100 #첫번째 row, 두번째 col의 요소를 100으로 변경
matrix
...
#일반적으로 쓰는 데이터는 문자변수, 숫자변수 등이 하나의 데이터셋에 담겨있음. 이 경우 쓰는 것이 dataframe. 앞으로 우리가 쓰는 대부분의 데이터는 dataframe일 것음

btsname<-c("RM","Jin","suga","jhope","jimin","V","JK")
btsyear<-c(1984,1992,1993,1994,1995,1995,1997)
...
#factor의 이해
#factor란 주로 categorical란 변수로서 "값"(일반벡터)에 "level"이라는 정보를 추가한 것
